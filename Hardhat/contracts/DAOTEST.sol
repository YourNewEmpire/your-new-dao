...
contract DAO is ERC1155URIStorage, ContextMixin, NativeMetaTransaction {
    ...
    mapping(uint256 => string) public svgMapping;
    modifier onlySeller() {
        require(
    ...
    // This will add the first token as id 0. With this, the user can control the token ids for minting new ones.
    ...
    function ownerMint(
        ...
        //todo - should require that the tokenString exists.
        _mint(to, tokenIdMapping[tokenString], amount, "");
    }

    function setSvg(uint256 tokenId, string svgString) public {
        //todo - add owner stuff later
        require(
            tokenIdArray.length <= tokenId,
            "Trying to set SVG for token ID that does not exist."
        );
        svgMapping[tokenId] = svgString;
    }

    function getSvg(uint256 tokenId) private view returns (string memory) {
        string memory svg = svgMapping[tokenId];
        return svg;
    }
